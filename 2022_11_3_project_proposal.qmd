...
Things to highlight:
1) impact & unpredictability of sea level rise
2) importance of biodiversity to ecosystems; increasing recognition of this
3) cities are realizing that it is more and more important to value and protect their biodiversity
4) this approach considers the potential utility of citizen science data as a tool to understand the threat of sea level rise to avian biodiversity in philadelphia


Saltwater vs. freshwater (see NOAA map)
- in certain scenarios, we could see saltwater intrusion *past* the dam at the Fairmount Waterworks (by the Art Museum)
...
![Bird Biodiversity Hotspots in the Philadelphia Region](C:\\Users\\Nissim\\Desktop\\Fall 2022\\Floodplain Management\\Final Project Data\\2022_11_03_ebird_phl_hotspots_screenshot.png){fig-align="center"}
...
First, we need to look at our raw number of observations year by year. This element is crucial because of the influence of pandemic birding. The extraordinary and unprecedented rise in citizen science data collection during the pandemic has been a boon for ecological data. However, it also complicates the interpretation of these data. As Sara Harrison explains in *Wired*, "Scientists can't always tell whether changes in the data are due to animal behavior or just an increase in the amount of information available. Furthermore, "It's not just that *more* people are observing---it's also a matter of *where* they are observing." Observations in urban areas have increased, while observations in rural areas have decreased, suggesting likely undersampling in less-accessible habitats.^[Sara Harrison, “Pandemic Bird-Watching Created a Data Boom-and a Conundrum,” Wired (Conde Nast, September 30, 2021), https://www.wired.com/story/pandemic-bird-watching-created-a-data-boom-and-a-conundrum/.]
...
ggplot(unique_per_chklst, aes(x = sampling_event_identifier, y = scientific_name, label = year)) +
    geom_smooth(method = "lm", se = FALSE, formula = y ~ x + I(x^2), aes(col = "Quadratic")) +
...
# grab geometry
phl_ebird_hex = phl_ebird_hex |>
                    mutate(nb_geom = st_geometry(phl_ebird_hex), #geoms for poly2nb
                           nb = st_contiguity(nb_geom), # generate neighbors
                           weights = st_weights(nb)) # weight matrices from neighbors

birds_hex_lines = nb2lines(phl_ebird_hex$nb, 
                          coords = st_centroid(phl_ebird_hex$geometry), 
                          as_sf = TRUE)

lisa = local_moran(phl_bird_hex_means$avg_unique_species, phl_ebird_hex$nb, phl_ebird_hex$weights, nsim = 999)


lisa = cbind(phl_bird_hex_means, 
                     lisa) |>
                      mutate(
...
tm_shape(lisa) +
  tm_fill(col = "lisa_map", palette = small_pal, alpha = 0.7, title = "LISA Clusters")+
  tm_borders(col = "white", lwd = 0.05) +
  tm_layout(frame = FALSE, main.title = "LISA")

moran.plot(lisa$avg_unique_species, nb2listw(phl_ebird_hex$nb))
```
...
Can I plot hexagon avg unique species vs. % SLR innundation for different SLR scenarios as a measure of severity? how to map that?
  - might be a good opportunity for a bivariate choropleth map--high biodiversity vs. high % innundation
...
overlapping_areas <- purrr::imap(
  all_polygons[1:length(all_polygons)],  ~calculate_overlap(.x, phl_bird_hex_means, .y)
  )
...
### pa low 0ft 
 ...
 tm_shape(phl_bird_hex_means) +
                 tm_polygons(col = 'polygon_1', palette = 'viridis', style = 'jenks') +
             tm_shape(all_polygons[[1]][phl_bounds, ]) +
                 tm_polygons(col = 'red')
...
2) species with sufficient observations
...
-misalignment of data sources
-difficult to get sufficient spatial resolution with citizen science data for mapping---observations are not consistently tied to specific enough points to be truly helpful for finely-tuned management
