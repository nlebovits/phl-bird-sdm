...
species from the same observation event. \[Don\'t forget citation\].
...
The primary focus of our analysis here is *species richness*: the total number
of species within our study region. To begin with, we can explore the number of
unique species observed in Philadelphia per year.
...
[^16]: Jimenez-Valverde et al., 2009
...
concern, "NT" to near-threatened species (a step below vulnerable species), and "VU" to vulnerable species.^[http://datazone.birdlife.org/species/spcredcat] Observations not matching any taxonomy in the IUCN list have been filtered out. As the graph shows, species of least concern have made up roughly 96-97% percent of observations in any given year, near threatened species another 1.5-2%, and vulnerable species around 1-1.5% (mouse over the interactive graph to see specific numbers for each year).
...
ggplotly(ggplot(data = (ebird_last_decade |> filter(x2022_iucn_red_list_category %in% c("LC", "NT","VU"))),
       aes(year, fill = x2022_iucn_red_list_category)) +
...
As above, we can once again plot observations against
vulnerable species as a function of effort, in an estimation of a rarefaction
curve.
...
spatial bias. As the COL explains,
...
In sum, we have estimated the total number of bird species
present in Philadelphia in any given year, also looking specifically at
vulnerable species. We have also mapped an estimated spatial distribution of
both total species richness and species richness for vulnerable species
specifically. The estimated rarefaction curve suggests that observations as a
function of effort are approaching an asymptote around 290 species as more and
more eBird data are reported. The maps indicate that areas along the Delaware
River and in South Philadelphia are key points of bird biodiversity, especially
...
Exploring NOAA data, it is important to distinguish between
sea level rise data and flooding data, as mentioned before. Although this
project only considers sea level rise data, flooding is intimately related to
sea level rise. As NOAA explain,
...
Ornithology Lab\'s approach. Similarly, in the visualizations, the categorical
breaks between low, medium-low, medium-high, and high sea level rise are set according
to Cornell\'s precedent.

As is evident in the maps below, sea level rise will
predominately affect south Philadelphia and the area along the Delaware in the
east. Additionally, there will be consequences up the Schhulkil River, stopping
at the Fairmount Waterworks dam.

Even under the 1 ft sea level rise scenario, areas
immediately adjacent to the hotspots identified above---FDR Park, John Heinz, and
Pennypack on the Delaware---will be substantially affected by sea level rise.
...
In statistics, cluster analysis helps to identify groupings
...
all the cells in our analysis.

As this histogram indicates, a small proportion of the cells
in the grid account for most of the biodiversity---a total of \[how many\] cells have
0 to 5 observations, while only \[how many\] have 50 or more. Therefore, we know
...
It\'s worth being mindful that these are not perfect proxies
for \"top\" hotspots, especially because any ranking bumps up against the issue
that the number of hexagons we have here is arbitrary.

Finally, we can plot biodiversity against sea level rise to
...
ggplot(phl_ebird_hex_means, aes(x = pct_SLR_1ft, y = avg_unique_species, col = pct_SLR_1ft, size = avg_unique_species)) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5)) +
  annotate("rect", xmin = 30, xmax = 100, ymin = 40, ymax = max(phl_ebird_hex_means$avg_unique_species),
           alpha = .2,fill = "#3262AB") +
  annotate("text", x = 65, y = 125, label = "High Biodiversity, \n High SLR")
```
...
Unfortunately, the data are messy---they violate most of the
basic assumptions of various kinds of clustering, such as k-nearest neighbors
and hierarchical clustering. Although I attempted various transformations to
normalize the data, I was never able to achieve \[what?\]

That being said, it seems that sophisticated clustering may
not be necessary. The data are messing, by the overlap of high biodiversity and
high sea level rise can be identified qualitatively using a very rudimentary
decision tree.

\[Can I draw this decision tree? Maybe in Illustrator?\]

Explain the quantile break versus Jenks breaks.

Finally, in the section below, I map
...
[^21]: https://www.cbd.int/doc/publications/cbd-ts-98-en.pdf
